...
public class PromptFactory {
    ...
    public static String buildPromptForMenuSelection(String menu, String userText) {
        return """
            Ты выбираешь из меню ТОЛЬКО те блюда, которые подходят под запрос пользователя.
            ...
            ЖЁСТКИЕ ПРАВИЛА:
            1. Если пользователь просит:
               - "без мяса"
               - "постное"
               - "постный"
               - "не мясное"
               - "вегетарианское"
               то НЕЛЬЗЯ включать блюда, где есть:
               - мясо
               - говядина
               - свинина
               - курица
            ...
            2. Если пользователь просит мясное или блюда с мясом, нужно возвращать ТОЛЬКО блюда, где есть:
               - мясо
               - говядина
               - свинина
               - курица
            ...
            3. Если запрос конкретный, НЕЛЬЗЯ возвращать всё меню целиком.

            4. Нужно вернуть только подходящие блюда.

            5. Ответ должен быть СТРОГО валидным JSON.

            6. Никакого текста вне JSON.

            7. Для каждого блюда верни только:
               - name
               - sizes

            8. sizes всегда должен быть массивом.
               Если размеров нет — верни пустой массив [].
            ...
            ЕСЛИ ПОДХОДЯЩИХ БЛЮД НЕТ:
            {
              "items": []
            }

            ПРИМЕР:
            Если пользователь написал "хочу постное, без мяса",
            то НЕЛЬЗЯ возвращать блюда с курицей, индейкой, говядиной, креветками, крабом, рыбой и прошутто.
